/* UnseenGo AI — Google Maps / Places photo enhancement
   Loads the complete photo set returned by the Google Places proxy.
*/
(function(){
 ...
 function galleryMarkup(imgs,name){return imgs.map((x,i)=>`<figure><img src="${esc(x.url)}" alt="${esc(name)} — photo ${i+1}" loading="lazy" title="${esc(name)} photo ${i+1}"></figure>`).join('')}
 ...
 async function enforceModal(){
   ...
   const name=title.textContent.trim(),city=locationEl.textContent.trim()||new URLSearchParams(location.search).get('city')||'';if(!name)return;
   const imgs=await photos(name,city);
   if(imgs[0]){
     holder.innerHTML='<img src="'+esc(imgs[0].url)+'" alt="'+esc(name)+' — Google Maps photo" loading="lazy">';
     const credit=document.getElementById('photoCredit');if(credit)credit.textContent='Photos from Google Maps / Places. Required attribution is shown when supplied by Google.';
   }
   if(gallery){
 ...
 function observe(){
   ...
   if(grid)new MutationObserver(()=>{if(grid.querySelector('.city-place-card'))enforceCards()}).observe(grid,{childList:true});
   const modal=document.getElementById('placeModal');
   if(modal)new MutationObserver(()=>{if(modal.classList.contains('open'))enforceModal()}).observe(modal,{attributes:true,childList:true,subtree:true});
   setTimeout(enforceCards,700);setTimeout(enforceCards,1800);
 }
