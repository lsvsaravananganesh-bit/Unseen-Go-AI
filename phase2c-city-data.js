...
(function () {
  ...
  // Calls the Supabase Edge Function created in Phase 2C.
  // It securely calls Google Places Text Search + Place Photos on the server.
  async function googlePlacePhotos(place, placeCity) {
    ...
    if (!sb) return null;
    try {
      const { data, error } = await sb.functions.invoke('google-place-photos', {
        body: { place, city: placeCity }
      });
      ...
      console.warn('UnseenGo AI: Google Places photo lookup failed.', e);
      return null;
    }
  }
  ...
  function attributionHtml(attributions) {
    ...
    return attributions.map(a => {
      const name = esc(a.displayName || a.uri || 'Google Maps contributor');
      const uri = a.uri ? esc(a.uri) : '#';
      return a.uri ? `<a href="${uri}" target="_blank" rel="noopener">${name}</a>` : name;
    }).join(', ');
  }

  async function enrichPlace(p, cardIndex) {
    if (!p || p.image_url) return null;
    const result = await googlePlacePhotos(p.name, dataset.city.name);
    if (!result) return null;

    p.google_place_id = result.id || p.google_place_id;
    p.latitude = result.latitude ?? p.latitude;
    p.longitude = result.longitude ?? p.longitude;
    p.map_url = result.mapsUrl || p.map_url;
    p.google_photos = result.photos || [];

    const first = p.google_photos[0];
    const card = document.querySelector(`.city-place-card[data-index="${cardIndex}"] .place-image`);
    if (card && first?.url) {
      card.innerHTML = `<img src="${esc(first.url)}" alt="${esc(p.name)}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\'image-fallback\'>✦<span>Photo unavailable</span></div>'">`;
    }
    return result;
  }
  ...
  function render(data) {
    ...
    // Populate missing photos from Google Places. Do this progressively so the page appears immediately.
    places.forEach((p, i) => { if (!p.image_url) enrichPlace(p, i); });
  }
  ...
  async function openPlace(i) {
    ...
    document.getElementById('modalLocation').textContent=c.name+(p.latitude&&p.longitude?` · ${Number(p.latitude).toFixed(4)}, ${Number(p.longitude).toFixed(4)}`:'');
    ...
    document.getElementById('modalPhoto').innerHTML=p.image_url?`<img src="${esc(p.image_url)}" alt="${esc(p.name)}">`:'<div class="modal-photo-loading">Finding Google Photos…</div>';
    document.getElementById('modalGallery').innerHTML='';
    document.getElementById('photoCredit').textContent='';

    const result = await googlePlacePhotos(p.name, c.name);
    if (result) {
      p.google_place_id = result.id || p.google_place_id;
      p.latitude = result.latitude ?? p.latitude;
      p.longitude = result.longitude ?? p.longitude;
    ...
    const photos = p.google_photos || [];
    if (p.image_url) {
      document.getElementById('modalPhoto').innerHTML=`<img src="${esc(p.image_url)}" alt="${esc(p.name)}">`;
    } else if (photos[0]?.url) {
      document.getElementById('modalPhoto').innerHTML=`<img src="${esc(photos[0].url)}" alt="${esc(p.name)}">`;
    } else {
      document.getElementById('modalPhoto').innerHTML='<div class="modal-photo-loading">No Google photo was returned for this place.</div>';
    }

    if (photos.length > 1) {
      document.getElementById('modalGallery').innerHTML=photos.slice(1,6).map(x=>`<figure><img src="${esc(x.url)}" alt="${esc(p.name)}" loading="lazy"></figure>`).join('');
    }
    const attribution = photos.flatMap(x => x.attributions || []);
    document.getElementById('photoCredit').innerHTML = attribution.length ? `Photo attribution: ${attributionHtml(attribution)}` : (photos.length ? 'Photos provided through Google Places.' : '');

    document.getElementById('modalMapLink').href=p.map_url||mapSearch(p.name,c.name);
    document.getElementById('modalMap').src=p.latitude&&p.longitude?`https://www.google.com/maps?q=${encodeURIComponent(p.latitude+','+p.longitude)}&output=embed`:mapEmbed(p.name,c.name);
  }
